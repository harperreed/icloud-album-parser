...
fn test_extension_from_mime_type() {
    ...
    assert_eq!(utils::extension_from_mime_type("application/octet-stream"), ".jpg");
}

#[test]
fn test_detect_mime_type() {
    // JPEG test data (FF D8 FF)
    let jpeg_bytes = [0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10, 0x4A, 0x46, 0x49, 0x46, 0x00, 0x01];
    ...
    let mp4_bytes = [0x00, 0x00, 0x00, 0x18, 0x66, 0x74, 0x79, 0x70, 0x6D, 0x70, 0x34, 0x32];
    assert_eq!(utils::detect_mime_type(&mp4_bytes, None), "video/mp4");
    
    // MOV test data (ftyp qt at position 4-10)
    let mov_bytes = [0x00, 0x00, 0x00, 0x18, 0x66, 0x74, 0x79, 0x70, 0x71, 0x74, 0x20, 0x20];
