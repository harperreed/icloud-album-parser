...
mod tests {
    ...
    async fn test_icloud_photos() {
        ...
        let _mock_redirect = server.mock("POST", "/webstream")
            .with_status(330)
        ...
        let response = mock_get_icloud_photos(token, &base_url, &mock_url).await.unwrap();
        ...
        assert_eq!(derivative.url, Some("https://example1.icloud.com/path/to/image1.jpg".to_string()));
